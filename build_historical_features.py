...
def load_gsax(season_year):
    """Load team goalie GSAx for a given season year (2023, 2024, 2025)."""
    try:
        df = pd.read_csv('data/team_goalie_gsax.csv')
        df = df[df['season_year'] == season_year][['team', 'team_goalie_gsax']].copy()
        # Map abbr to full name
        df['team'] = df['team'].map(abbr_to_full).fillna(df['team'])
        df = df.dropna(subset=['team'])
        # Handle multi-team rows (e.g. "ANA,BUF") — skip those
        df = df[~df['team'].str.contains(',', na=False)]
        df = df.groupby('team')['team_goalie_gsax'].sum().reset_index()
        print(f"  GSAx loaded: {len(df)} teams for season_year={season_year}")
        return df.set_index('team')['team_goalie_gsax']
    except Exception as e:
        print(f"  GSAx load failed: {e}")
        return None
...
def add_features(games, team_stats, gsax_series=None):
    games = games.copy()
    ...
    # GSAx (goalie quality)
    if gsax_series is not None:
        games['home_gsax'] = games['home_team'].map(gsax_series)
        games['away_gsax'] = games['away_team'].map(gsax_series)
        games['gsax_diff'] = games['home_gsax'] - games['away_gsax']
        matched = games['gsax_diff'].notna().sum()
        print(f"  GSAx matched {matched}/{len(games)} games")
...
gsax_2324 = load_gsax(2023)
df_2324 = add_features(load_games('data/games_2324.csv'), load_mp('data/mp_2023.csv'), gsax_2324)
df_2324['season'] = '2023-24'
print(f"  2023-24: {len(df_2324)} games")

gsax_2425 = load_gsax(2024)
df_2425 = add_features(load_games('data/games_2425.csv'), load_mp('data/mp_2024.csv'), gsax_2425)
df_2425['season'] = '2024-25'
print(f"  2024-25: {len(df_2425)} games")

gsax_curr = load_gsax(2025)
df_curr = add_features(load_games('data/games.csv'), load_mp('data/moneypuck.csv'), gsax_curr)
...
print(f"GSAx coverage: {combined['gsax_diff'].notna().sum()}/{len(combined)} games")
...
gsax_cols = [c for c in combined.columns if 'gsax' in c]
print(f"GSAx features: {gsax_cols}")